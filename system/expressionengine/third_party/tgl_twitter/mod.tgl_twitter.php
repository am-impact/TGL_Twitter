...
class Tgl_twitter
{
	...
	function __construct()
	{
		...
		$this->exclude_replies = $this->EE->TMPL->fetch_param('exclude_replies', '');
		...
		if ($this->limit)
		{
			// if reply's are excluded you can get less results
			$this->parameters['count'] = $this->limit * 2;
		}

		if ($this->exclude_replies)
		{
	...
	function _fetch_remote()
	{
		...
		$oauth         = new TwitterOAuth($settings['consumer_key'], $settings['consumer_secret'], $settings['oauth_token'], $settings['oauth_token_secret']);
		$oauth->format = $this->api_version != '1' ? 'json' : 'xml';

		$params = array_merge($this->parameters, array('include_rts' => 'true', 'include_entities' => 'true'));
